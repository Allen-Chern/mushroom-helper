...
3. **申請 reCAPTCHA v3 site key**(給 App Check 用,擋掉非正式前端的呼叫):前往 [reCAPTCHA 管理後台](https://www.google.com/recaptcha/admin/create),註冊一個 v3 key,類型選「Score based (v3)」。網域要把 Firebase Hosting 的**兩個**網域都加進去:`<專案ID>.web.app` 和 `<專案ID>.firebaseapp.com`(兩個都指向同一個網站,只加一個的話用另一個網域開啟的人會驗證失敗)
4. **Firebase Console 啟用 App Check**:左側選單「App Check」→ 選這個 Web app → Provider 選 **reCAPTCHA**(不是「reCAPTCHA Enterprise」,兩個是不同選項)→ 貼上上一步的 site key
...
7. 本機開發時,`npm run dev` 會自動接到本機的 Functions emulator(不需要每次改程式都重新部署),但第一次執行需要:
   ...
   ⚠️ **注意**:emulator 裡的 function 仍會呼叫「真的」Vision API(不是本機模擬),需要本機先跑過 `gcloud auth application-default login` 設好憑證。如果開發機沒裝 `gcloud` CLI,這條路走不通,可以改成直接 `firebase deploy --only functions` 部署到正式環境測試

8. **目前 App Check 的實際狀態**:`functions/index.js` 裡 `ocrRecognize` 的 `enforceAppCheck` 目前是 `false`(暫時停用強制),因為遇到 reCAPTCHA 驗證失敗的已知問題還沒排除,詳見 [`docs/infrastructure.md`](./docs/infrastructure.md) 的已知問題章節。也就是說上面 3-4 步驟設定完後,App Check 目前**不會**真的擋下未授權呼叫,只是先把設定準備好
